"""
Seed PlacementQuestions + PlacementOptions from JSON (placement_questions.json next to this script).
Target schema: data/db/prod/VocabifySchema.sql.
...
# Script lives in data/placement/ next to placement_questions.json
DEFAULT_JSON = Path(__file__).resolve().parent / "placement_questions.json"
...
def placement_attempt_count(cur, attempts_has_is_deleted: bool) -> int:
    if attempts_has_is_deleted:
        cur.execute("SELECT COUNT(*) FROM dbo.PlacementAttempts WHERE IsDeleted = 0")
    else:
        cur.execute("SELECT COUNT(*) FROM dbo.PlacementAttempts")
    return int(cur.fetchone()[0])


def active_question_count(cur, pq_has_is_deleted: bool) -> int:
    if pq_has_is_deleted:
        cur.execute(
            "SELECT COUNT(*) FROM dbo.PlacementQuestions WHERE IsDeleted = 0 AND IsActive = 1"
        )
    else:
        cur.execute("SELECT COUNT(*) FROM dbo.PlacementQuestions WHERE IsActive = 1")
    return int(cur.fetchone()[0])
...
def main() -> int:
    ...
    cur = conn.cursor()
    pq_has_del = has_column(cur, "PlacementQuestions", "IsDeleted")
    po_has_del = has_column(cur, "PlacementOptions", "IsDeleted")
    pa_has_del = has_column(cur, "PlacementAttempts", "IsDeleted")

    try:
        if active_question_count(cur, pq_has_del) > 0 and not args.force:
            print(
        ...
        if args.force:
            n_attempts = placement_attempt_count(cur, pa_has_del)
            if n_attempts > 0:
                print(
            ...
            for q in block.get("questions") or []:
                ...
                if pq_has_del:
                    ...
                    cur.execute(
                        """
                        INSERT INTO dbo.PlacementQuestions (
                            LevelId, QuestionText, Explanation, IsActive
                        )
                        OUTPUT INSERTED.Id
                        VALUES (?, ?, ?, 1)
                        """,
                ...
                for sort_order, opt in enumerate(q.get("options") or [], start=1):
                    ...
                    # One point per correct answer; wrong options always 0
                    ...
                    if po_has_del:
                        ...
                        cur.execute(
                            """
                            INSERT INTO dbo.PlacementOptions (
                                QuestionId, OptionText, IsCorrect, Score
                            )
                            VALUES (?, ?, ?, ?)
                            """,
                            (qid, otext, is_correct, score),
